...
func TestAddStudentStringUnconstrained(t *testing.T) {
	var students []String
	result := addStudent2(students, "Udin")
	result = addStudent2(result, "Khoirudin")
	result = addStudent2(result, "Rosyadi")
	assert.Equal(t, 3, len(result))
	assert.Equal(t, "String", reflect.TypeOf(result[0]).Name())
	assert.Contains(t, "Udin", result[0])
	assert.Contains(t, "Khoirudin", result[1])
...
func TestAddStudentIntUnconstrained(t *testing.T) {
	students1 := []Integer{}
	result1 := addStudent2(students1, 45)
	result1 = addStudent2(result1, 64)
	result1 = addStudent2[Integer](result1, 78)
	assert.Equal(t, 3, len(result1))
	assert.Equal(t, "Integer", reflect.TypeOf(result1[0]).Name())
	assert.EqualValues(t, 45, result1[0])
	assert.EqualValues(t, 64, result1[1])
...
func TestAddStudentUnconstrained(t *testing.T) {
	students := []Student{}
	result := addStudent2(students, Student{"John", 213, 17.5})
	result = addStudent2(result, Student{"John", 214, 17.5})
	result = addStudent2(result, Student{"John", 215, 17.5})
	assert.Equal(t, 3, len(result))
	assert.Equal(t, "Student", reflect.TypeOf(result[0]).Name())
	assert.EqualValues(t, 213, result[0].ID)
	assert.EqualValues(t, 214, result[1].ID)
